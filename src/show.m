...
%     set(gcf, 'Position',[100 100 900 600]);
    ...
    for k = 1:numel(betas)
        ...
        apply(@contour, s, 'Psi', psi, 15, s.Vinf * linspace(-1, 1, 55));
        sphere();
        axis equal;
        axis([-1 1 -1 1]*10)        
        hold off;
        txt = sprintf('beta = %.2e, gamma = %.5f (analytical)', betas(k), g);
        title(txt);
        print('-depsc2', ['figures/' txt '.eps'])
        ...
        axis([-1 1 -1 1]*10)        
        hold off;
        txt = sprintf('beta = %.2e, gamma = %.5f (numerical)', betas(k), g);
        title(txt)
        print('-depsc2', ['figures/' txt '.eps'])
