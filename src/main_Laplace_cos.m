function [sol] = main_Laplace_cos(alpha)
    clc;
    g = grids(1e3, 100);
    ...
    semilogx(convn(r, [1;1]/2, 'valid'), ...
        diff(phi, 1) ./ repmat(diff(r), [1 size(phi, 2)]), g.r(end), betas(end)*cos(t(2:end-1)), '.')
end
...
function flux = charge(sol)
    ...
    fluxT = Deriv(g, 'sin(t)' * DPhi_Dt, 2) * '1/sin(t)';
    flux = fluxR + fluxT;
end
