...
for k = 1:3
    for j = 2
        ...
        u = s.Vr; % numerical solution
        e = abs(s.Vr - U1(k) * s.b); % deviation from linear regime
...
%         title(t, 'FontSize', 24)
        ...
        legend boxoff;
        grid on;    
        print('-depsc2', fullfile(p, sprintf('departure_from_linear_%s.eps', f)))
...
%         title(t, 'FontSize', 24)
        ...
        legend boxoff;
        grid on;    
        print('-depsc2', fullfile(p, sprintf('numerical_vs_analytical_%s.eps', f)))
    end
end
