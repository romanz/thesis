function [] = main_test()
    init = [];
    g = grids(1e4, 256+1, 128+1, 1);
    betas = 0.1 .^ (2:5);
    
    if isempty(init)
        init.Phi = zeros(g.Phi.size);
        init.C = zeros(g.C.size);
    end
...
function [op] = boundary_conditions(sol)
    ...
    g1 = g.crop(0, 1);
    phi = Boundary(sol.Phi, -1, 2);
    c = Boundary(sol.C, -1, 2);    
    bnd{end+1} = Interp(g1, phi - log(c+1)); 
    bnd{end+1} = Deriv(g1, phi + log(c+1), 1); 
    
    g = sol.grid.Vr;
    g = Grid(g.r(end), g.t(2:end-1));
    Phi_inf = @(r,t) sol.beta;
    Phi = Interp(g, Boundary(sol.Phi, 1, 2));
    C = Interp(g, Boundary(sol.C, 1, 2));

    bnd{end+1} = Phi - Phi_inf;
    bnd{end+1} = C; % R=inf
...
%     zeta = log(Interp(g, sol.C)) - log(sol.gamma);
%      
...
function [eqn] = system_equations(sol)    
    ...
    eqn = Join(charge(sol), salt(sol));
%     eqn = charge(sol);
end

function dbg = debug_solver(sol)
    Er = Deriv(sol.grid.Vr, sol.Phi, 1);
    Et = Deriv(sol.grid.Vt, sol.Phi, 2) * '1/r';
    dbg.Er = Crop(Er, [0 1]);
    dbg.Et = Crop(Et, [1 0]);
end
