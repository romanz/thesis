...
\title{Asymptotic and Numerical Solution for Electro-Kinetic Flow Around an Ion-Selective Particle}
...
\abstract{A numerical solver for the simulation of an electrokinetic migration of particles in an
electrolyte solution is presented. The electrokinetic transport process is
described by a nonlinear partial differential equation system, 
as well as nonlinear boundary conditions.
The nonlinear system is discretized and solved iteratively using Newton's method.
...
\\
\delta^2 &=& \frac{\eps^* \varphi^*}{c^* e^* z (a^*)^2} = 
\pars{\frac{\delta^*}{a^*}}^2.
\end{eqnarray}
...
The electro-kinetic flow is described by the following nondimensional equations:
...
We employ the reference frame of the ion exchanger.
...
The chemical properties of the particle's surface $\mathcal{S}$ (defined at $r=1$ with normal $\bn = \brhat$), determine the boundary conditions on $\mathcal{S}$.
...
(see full derivation in \cite{yariv2010asymptotic}), 
...
\\ 
&&- \frac{\left(3 \cos\theta - 5 {\cos}^3\theta\right)}{r^4} 
\left( - \frac{19 {\cU_1}^2\alpha^2}{5120} + \frac{97 \cU_1\alpha}{5120} + 
\frac{3 \cU_2\alpha}{320} + \frac{21}{1280}\right) \\
&& + \frac{{\cos}^3\theta}{r^2} \left(\frac{15 \cU_1\alpha}{64} + \frac{3}{32}\right)  
+ \frac{{\cos}^3\theta}{r^6} \left(\frac{\cU_1\alpha}{64} + \frac{3}{64}\right),
\\
C_3 &=&
...
&&- \left(\frac{59 {{\cU_1}}^2\alpha^2}{5120} + \frac{103 {\cU_1}\alpha}{5120} + \frac{69 {\cU_2}\alpha}{320} + \frac{3}{1280}\right)\frac{\left(3 \cos\theta - 5 {\cos^3\theta}\right)}{r^4} \\
...
r^2 {\sin^2\theta} \left(\frac{{\cW_2}}{15} - \frac{{\cW_1}}{3} + \frac{209}{3360}\right) 
 - \frac{{\sin^2\theta}}{r} \left(\frac{5 {\cW_2}}{3} - \frac{{\cW_1}}{3} + \frac{35}{264}\right)
...
\\ && + \frac{\left(\frac{2 {\cW_2}}{5} + \frac{829}{28160}\right) \left(4 {\sin^2\theta} - 5 {\sin^4\theta}\right)}{r^3} + \frac{{\sin^2\theta} \left(\frac{{\cU_1}\alpha}{16} - \frac{1}{8}\right) \left(2 r^3 - 3 r^2 + 1\right)}{4 r},
\\
