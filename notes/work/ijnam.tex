...
The numerical results are obtained for an ion-exchangers and 
surface charged inert particles. The numerical results 
...
In order to have square system, we should remove one conservation equation at this grid point too.
In case of the pressure $P$, we remove one mass conservation equation,
$\bnabla \cdot \bV = 0$, at this grid point. 
In case of the electric potential, we remove the charge conservation
equation, $\bnabla \cdot \pars{C \bnabla \varPhi} = 0$, at this grid point.
For $\br = \cO(\bx)$ and $\bG = \bnabla \cO(\bx)$, the new square linear system read:
\begin{equation}
...
whre the update step is $\Delta \bx = \mathbb{P} \cdot \delta \bx$.
...
For each $\beta$ value, 
Newton solver is initialized with the linear solution and is iterated $K$ times.
...
The numerical solver uses $N_r \times N_\theta = (128 \times 128)$ and $(256 \times 256)$
for Richardson extrapolation, $R_{\max} = 10^2$,
$K = 3$, $M = 5$ and $\beta = 0.1 \cdot 2^{t}$ for $t = -2:0.5:5$.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
\section{Discussion} \label{sec:discussion}
A numerical framework for solver construction for 
...
\begin{figure}[htbp]
	...
	\includegraphics[width=0.45\textwidth]{figs/Cubic/Stream/N_0.9000_0.3981.eps}
	\includegraphics[width=0.45\textwidth]{figs/Cubic/Stream/A_0.9000_0.3981.eps}
	\includegraphics[width=0.45\textwidth]{figs/Cubic/Stream/N_0.9000_1.0000.eps}
	\includegraphics[width=0.45\textwidth]{figs/Cubic/Stream/A_0.9000_1.0000.eps}
        ...
        Note that there is good correspondence between the numerical
        results and the analytical solution for $\beta < 1$, validating
        the vortex formation. For $\beta \ge 1$, the cubic analytical
        solution does not match the numerical results, probably due
        to further nonlinear terms in the solution.
        }
    \label{fig:Vortex}
    \end{center}
\end{figure}
