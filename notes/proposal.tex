...
dynamics can be found \cite{Yariv10a}.

Electrokinetics is described by the following physical phenomena (in dimensionless formulation):
\begin{enumerate}
\item Electrostatic potential---the fluid bulk contains no free
charge:
...
\end{equation}
\end{enumerate}
{\bf IY: WRITE EXPLICITLY HERE WHAT THE VARIABLES ARE---C,P,V,
ETC.}The boundary conditions are determined by the specific
...
axisymmetric setting \cite{Yariv10b} but it is hard to extend this
...
{\bf IY: ADD HERE A PARAGRAPH THAT DISCUSSES THE IMPORTANCE AND
RELEVANCE OF THE PROBLEM---YOU CAN TAKE THIS FROM THE PAPERS YOU
HAVE READ.}
...
electric fields, non-spherical particles, and asymmetric charge on
the particle surface. We expect to thus gain insight into the
chemical and physical behavior in far more general regimes than
are currently well-understood. We hope that this will also lead to
further analytical developments, based on new scaling behavior
...
micro-swimmers \cite{Paxton04, Howse07}.
...
The problem configuration is taken from \cite{Yariv10b} and
...
\cite{Yariv10b}, using the Dukhin-–Derjaguin slip formula for
...
\cite{Yariv10b}. Using a $60 \times 15$ grid for $(R,\theta) \in
...
Vanka-type \cite{Vanka86} preconditioner is used for equation
(\ref{eq:Stokes}). The preconditioners are computed as sparse
matrices, facilitating the relaxation process (\ref{eq:Relax}).

Vector extrapolation methods \cite{Sidi90} have been implemented
and are used to accelerate the solver's convergence. Later, a
Multigrid framework \cite{Brandt77, Yavneh06} will be introduced
to allow the efficient solution of large problems.
