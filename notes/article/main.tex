...
The goal of this work is to implement an iterative numerical
...
The source code of solver can be found at \cite{source}.
...
The resulting $\cU(\beta)$ is plotted using a log-log graph, so the asymptotic behaviour
of the solution could be examined for a wide range of $\beta$ values.

In order to validate the numerical solver, an asymptotic analysis of the nonlinear system
is performed as described in \ref{app:asymp}. The numerical results are compared to 
to the asymptotic solutions, and good correspondence is achieved.

\subsection{Linear Regime}
For $\beta \ll 1$, the linear regime takes place (for derivation, see \ref{app:linear}):
\begin{eqnarray*}
\cU = 2\log\pars{\frac{1 + \gamma^{-\frac{1}{2}}}{2}}\beta.
\end{eqnarray*}
Thus, for $\gamma > 1$, the Debye layer has positive space charge, corresponding to a negative
particle charge -- yielding negative drift velocity, $\cU < 0$. 
For $\gamma < 1$, we have $\cU > 0$, respectively.

The linear regime is simulated for $\gamma > 1$ (at Figure \ref{fig:Linear1})  
and for $\gamma < 1$ (at Figure \ref{fig:Linear2}).

\subsection{Cubic Regime}
When $\gamma \approx 1$, the cubic regime  (for derivation, see \ref{app:cubic}), where
\begin{eqnarray*}
\cU = O(\beta^3), 
\end{eqnarray*}
...
The switch from the linear regime to the cubic one happens at a critical electric
field value
\begin{eqnarray*}
\beta_c = O(\sqrt{|\gamma - 1|}).
\end{eqnarray*}

The numerical results, as well as the analytical ones, are given at Figure
\ref{fig:Cubic1} (for $\gamma > 1$) and at Figure \ref{fig:Cubic2} (for $\gamma < 1$).
Since log-log scale is employed, positive and negative velocities are shown in
...
as demonstrated by the numerical simulation and the asymptotic analysis results 
in Figure \ref{fig:Vortex}.

\subsection{Discussion}
The numerical solver results have good correspondence to the asymptotic linear model as
shown above.
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=2.00.eps}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=5.00.eps}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=10.00.eps}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=100.00.eps}
...
\begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=0.50.eps}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=0.20.eps}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=0.10.eps}
    \includegraphics[width=0.45\textwidth]{figs/Linear/g=0.01.eps}
    \caption{Steady-state velocity $\cU$ as a function of the 
        electric field magnitude $\beta$, for $\gamma < 1$. 
...
The solution to the nonlinear system can be written as a Taylor series in $\beta$.
\begin{eqnarray*}
