...
is performed as described in \ref{sec:asymp}. The numerical results are compared to 
...
The linear regime is simulated for $\gamma > 1$ (at Figure \ref{Linear1})  
and for $\gamma < 1$ (at Figure \ref{Linear2}).

\subsection{Cubic Regime}
When $\gamma \approx 1$, the cubic regime, $\cU = O(\beta^3)$, 
...
\ref{} (for $\gamma > 1$) and at Figure \ref{} (for $\gamma < 1$).
...
\end{eqnarray*} 
This sign change creates a vortex at the downstream part of the ion-exchanger, 
